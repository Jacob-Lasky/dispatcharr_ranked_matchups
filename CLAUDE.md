...
scoring.py        ← GameSignals + score_game + per-signal weight contributions
  ↓ matched by
matcher.py        ← EPG-programme / channel-name / stream-name candidates, resolved
                    by regex tiers + Claude fallback when ambiguous
  ↓ written by
plugin.py         ← creates virtual Channels + inactive XMLTV EPGSource + ProgramData
```
...
| `recording.py` | Auto-record policy (#216), pure and ORM-free: `resolve_slot_budget` (tightest positive M3U `max_streams` minus reserve), `plan_recordings` (first-fit slots, earliest kickoff first; busy = other recordings, held = ours in progress, never displaced), `classify_existing` (user-deleted -> tombstone, terminal -> skip), `decide` (create / full-save update / extend-only while recording), `cancellable`, `recording_over` (the 🔴, read from real rows), and the `recordings_state.json` load/save/prune. `MARKER_KEY` in `Recording.custom_properties` is the only "ours" test. The ORM around it is `_autorecord_*` in plugin.py, which writes with `.create()` / FULL `.save()` on purpose (the Recording signals are wanted; `.update()` or a narrow `update_fields` leaves the capture task stale). |
...
- `recordings_state.json` — auto-record bookkeeping (#216): recordings the plugin created, and tombstones for ones the user deleted so they are not re-created
